      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          ...
          <el-button
              ...
              type="text"
              icon="el-icon-delete"
              v-if="!checkedList.includes(scope.row.courseId)"
...
import {
  addOrUpdateCourseApi, deleteChoiceByStudentIdApi,
  deleteCourseByIdsApi, getChoiceByStudentIdApi, queryChoiceByStudentIdApi,
...
const getCheckedList = async() => {
  const res = await queryChoiceByStudentIdApi()
  checkedList.value = res.data.map(item => item.courseId);
  console.log('cccc',checkedList.value,res.data)
}
const getList = async () => {
  loading.value = true;
  try {
...
const handleDelete = async (row) => {
  ...
  ElMessageBox.confirm(
      ...
      .then(() => {
        getChoiceByStudentIdApi({courseIds: deleteIDs}).then((res) => {
          if(res.data === 'success'){
            ElMessage({
              type: 'success',
              message: '操作成功',
...
const handleDelete1 = async (row) => {
  ...
  ElMessageBox.confirm(
      ...
      .then(() => {
        deleteChoiceByStudentIdApi({choiceIds: deleteIDs}).then((res) => {
          if(res.data == true){
            ElMessage({
              type: 'success',
              message: '操作成功',
